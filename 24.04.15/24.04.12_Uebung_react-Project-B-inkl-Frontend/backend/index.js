...
app.get("/api/v1/todos/:id", (req, res) => {
  ...
  readTodos() // alle ToDos lesen
    .then(
      (todos) => todos.find((singleToDo) => singleToDo.id == todoId) // in allen ToDos die ID aus dem Request finden
    )
    .then((foundTodo) => res.status(200).json(foundTodo || {})) // Daten der gefundenen ID zurückgeben; falls kein Inhalt vorhanden, dann leeres Array zurüclgeben
    .catch((err) =>
