...
// 4. Update-Funktion, um gefilterte Sänger ins HTML zu schreiben
// 5. Filter-Funktion, um nach gesuchten Sängern filtern zu lassen und die gefilterten Sänger im HTML auszugeben
...
// ! Update-Funktion, um die gesuchten Werte auszugeben:
const updateFunc = (singer) => {
  // Container leeren, damit HTML ersetzt und nicht ergänzt wird:
  tableContainer.innerHTML = "";

  // Überschriften Zeile wieder einfügen:
  tableContainer.innerHTML += `
  <div><b>Name</b></div>
  <div><b>Country</b></div>
  <div><b>Period Active</b></div>
  ...
  // gefiltertes array im HTML ausgeben lassen:
  singer.forEach((item) => {
    tableContainer.innerHTML += `
    <div>
      <p>${item.name}</p>
...
const handleSearchClicked = () => {
  ...
  // gefilterte Sänger mit updateFunc im HTML ausgeben lassen:
  searchedSingers.length <= 0
    ? (tableContainer.innerHTML =
        "<p class='error'>Singer not found, try again</p>")
    : updateFunc(searchedSingers);
};
